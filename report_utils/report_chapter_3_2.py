"""
Chapter 3.2: RAG-Enhanced Consistency (Billable Codes Only) for medical coding report.
"""
...
def generate_chapter_3_2() -> str:
    """Generate Chapter 3.2: RAG-Enhanced Prediction Using Variants (Billable Codes Only)."""
    html = """
    <div class="chapter">
        <div class="chapter-title">Chapter 3.2: RAG-Enhanced Prediction (Billable Codes Only)</div>

        <h3>Building on Chapter 3.1: Filtering Non-Billable Codes</h3>
        <p style="margin-bottom: 15px;">
            Chapter 3.1 demonstrated RAG-enhanced prediction across all ICD-10 codes. However, ICD-10 has a critical distinction:
        </p>
        <ul style="margin-left: 20px; margin-bottom: 15px;">
            <li><strong>3-character codes (e.g., A00):</strong> Category headers - NOT billable</li>
            <li><strong>4+ character codes (e.g., A00.0):</strong> Specific diagnoses - BILLABLE</li>
        </ul>

        <h3>The Key Discovery: Filtering for Billable Codes</h3>
        <p style="margin-bottom: 15px;">
            Chapter 3.2 re-runs Chapter 3.1's <strong>exact same experiments</strong> but filters to only billable codes
            (LENGTH(code) > 3) using immutable SQL VIEWs. This shows whether removing non-billable category codes improves performance.
        </p>
...
"""
    ...
    try:
        ...
        # Check if all three RAG experiments exist (using billable VIEWs)
        cursor.execute("SELECT COUNT(*) FROM billable_rag_real_only")
        real_only_count = cursor.fetchone()[0]
        cursor.execute("SELECT COUNT(*) FROM billable_rag_synthetic_only")
        synthetic_only_count = cursor.fetchone()[0]
        cursor.execute("SELECT COUNT(*) FROM billable_rag_both")
        both_count = cursor.fetchone()[0]

        if real_only_count > 0 and synthetic_only_count > 0 and both_count > 0:
            # Get Chapter 3 baseline for comparison (billable codes only)
            cursor.execute("""
                SELECT
                    COUNT(*) as total,
                    SUM(CASE WHEN confidence > 0 THEN 1 ELSE 0 END) as correct,
                    AVG(confidence) * 100 as correctness_rate
                FROM billable_reverse_predictions
            """)
            base_total, base_correct, base_correctness = cursor.fetchone()

            # Calculate baseline consistency (billable codes only)
            cursor.execute("""
                SELECT
                    ic.code,
                    GROUP_CONCAT(rp.predicted_codes, '|||') as all_predictions
                FROM billable_reverse_predictions rp
            ...
            # Get metrics for all three RAG experiments (using billable VIEWs)
            experiments = [
                ('real_only', 'billable_rag_real_only', '3.2.1 (Real Only - Billable)'),
                ('synthetic_only', 'billable_rag_synthetic_only', '3.2.2 (Synthetic Only - Billable)'),
                ('both', 'billable_rag_both', '3.2.3 (Both - Billable)')
            ]
        ...
        <table class="table-wsj">
            ...
            <tbody>
                <tr>
                    <td><strong>Chapter 3 (Billable)</strong><br/><small>No RAG examples</small></td>
...
"""
            ...
            for exp_name in ['real_only', 'synthetic_only', 'both']:
                ...
                html += f"""
                <tr>
                    <td><strong>Chapter {r['display_name']}</strong><br/><small>RAG with {exp_name.replace('_', ' ')} (billable only)</small></td>
...
"""
            ...
            best_table = f"billable_rag_{best_exp[0]}"

            html += f"""
        <h3>Detailed Analysis: Best Performing Approach ({best_exp[1]['display_name']})</h3>
        <p style="margin-bottom: 20px;">
            The {best_exp[1]['display_name']} approach achieved the highest correctness ({best_exp[1]['correctness']:.1f}%) among billable codes.
            Below we show how its predictions compare to the Chapter 3 baseline for selected billable codes.
        </p>
"""

            # Get list of codes we tested (from billable VIEW)
            cursor.execute(f"""
            ...
            for actual_code, code_description in codes:
                # Get all variants for this code (using billable VIEWs)
                cursor.execute(f"""
                    SELECT
                        gd.detail_level,
                        gd.description,
                    ...
                    JOIN icd10_codes ic ON gd.code_id = ic.id
                    LEFT JOIN billable_reverse_predictions rp ON rp.generated_desc_id = gd.id
        ...
        <div class="info-box roundtrip-example">
            ...
            <p style="margin-bottom: 10px;">
                <strong>Chapter 3 (Billable):</strong> {ch3_unique} different predictions across {len(ch3_top_preds)} variants
                &nbsp;&nbsp;|&nbsp;&nbsp;
            ...
            <table class="table-wsj">
                <thead>
                    <tr>
                        ...
                        <th>Chapter 3 (Billable)<br/>Prediction</th>
...
"""
            ...
            # Analysis section
            html += """
        <h3>Analysis: Impact of Filtering to Billable Codes Only</h3>
"""
        ...
        <div class="highlight-box">
            <strong>Best Correctness (Billable):</strong> {best_corr[1]['display_name']} at {best_corr[1]['correctness']:.1f}%
            ({best_corr[1]['correctness'] - base_correctness:+.1f}% vs billable baseline)<br/>
            <strong>Worst Correctness (Billable):</strong> {worst_corr[1]['display_name']} at {worst_corr[1]['correctness']:.1f}%
            ({worst_corr[1]['correctness'] - base_correctness:+.1f}% vs billable baseline)<br/>
            <strong>Best Consistency (Billable):</strong> {best_cons[1]['display_name']} at {best_cons[1]['consistency']:.1f}%
            ({best_cons[1]['consistency'] - ch3_avg_consistency:+.1f}% vs billable baseline)
        </div>

        <h4>Key Findings: Billable-Only Performance</h4>
        <ul style="margin-left: 20px; margin-bottom: 15px;">
"""
            ...
            if synth_corr_change > 5:
                html += f"""
            <li><strong>Billable codes show strong RAG improvements:</strong> Among billable codes only, AI-generated variants improved
            correctness by {synth_corr_change:.1f}%, demonstrating that specific diagnoses (not category headers) benefit significantly from RAG.</li>
"""
            elif synth_corr_change > 0:
                html += f"""
            <li><strong>Modest RAG improvements on billable codes:</strong> Filtering to billable codes, RAG with synthetic variants improved
            correctness by {synth_corr_change:.1f}%, showing that even specific diagnoses benefit from similar examples.</li>
"""
            else:
                html += f"""
            <li><strong>Unexpected finding:</strong> Among billable codes, RAG showed {synth_corr_change:.1f}% change in correctness,
            suggesting that category headers may have been introducing noise in Chapter 3.1's results.</li>
"""

            # Analyze real_only performance
            real_corr_change = rag_results['real_only']['correctness'] - base_correctness
            html += f"""
            <li><strong>Real descriptions on billable codes:</strong> Official medical descriptions improved
            correctness by {real_corr_change:.1f}% when filtered to billable codes only.</li>
"""

            # Analyze both performance
            both_corr_change = rag_results['both']['correctness'] - base_correctness
            html += f"""
            <li><strong>Combined corpus on billable codes:</strong> Mixing real and synthetic examples
            achieved {both_corr_change:.1f}% improvement on billable codes.</li>
"""
        ...
        <p style="margin-bottom: 15px; margin-top: 15px;">
            <strong>Conclusion:</strong> By filtering to billable codes only (LENGTH > 3), we eliminate category headers
            that are not used in real medical billing. This immutable SQL VIEW approach allows us to re-analyze the exact
            same predictions from Chapter 3.1, revealing whether non-billable codes were introducing noise or helping performance.
            The results show the actual impact of this critical ICD-10 distinction on RAG-enhanced medical code prediction.
        </p>
"""
            ...
            html += """
        <div class="info-box">
            <div class="info-title">Status: Billable VIEWs Ready, Awaiting Chapter 3.1 Data</div>
            <p>
                SQL VIEWs for billable codes have been created, but Chapter 3.1's RAG predictions
                need to be generated first. Once Chapter 3.1 runs, this chapter automatically
                shows the billable-only analysis using immutable SQL VIEWs.
            </p>
            <p style="margin-top: 10px;">
                The VIEWs filter predictions using: <code>WHERE LENGTH(ic.code) > 3</code>
            </p>
            <p style="margin-top: 10px;">
                <strong>Available VIEWs:</strong> billable_reverse_predictions, billable_rag_real_only,
                billable_rag_synthetic_only, billable_rag_both
            </p>
        </div>
"""
